...
NLPROC_SOURCE =     cfg["NLPROC_SOURCE"]
TARGET_DUMP =       cfg["TARGET_DUMP"]
USE_EVERYTHING =    cfg["USE_EVERYTHING"]


# Load Corpus Data (from raw)
_,_,files =  os.walk(cfg["ECB_COMBINED_PATH"]).next()

raw_corpus_data = []
for fname in files:
    full_path = os.sep.join([cfg["ECB_COMBINED_PATH"], fname])
...
def dump(stuff, stuffs):
    ...
    fstream = open(os.sep.join([TARGET_DUMP,
                                "ECB-hyperpickled_%s.pkl"%`stuffs`]), "wb")
